...
def validate_fitnotes_dataframe(df):
    ...
    required_columns = ['Date', 'Exercise', 'Category', 'Weight', 'Weight Unit', 
    ...
    if df.empty:
        raise ValueError("The file is empty. Please upload a FitNotes export with workout data.")
...
def convert_fitnotes_to_hevy(df, mappings, timezone_offset=TIMEZONE_OFFSET_HOURS, 
    ...
    df['Date'] = (pd.to_datetime(df['Date'] + ' ' + workout_time) - 
